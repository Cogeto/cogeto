  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-50">
      ...
      >
        ...
        />
        <p className="mb-6 text-center text-sm text-slate-500">{t('demoLogin.heading')}</p>

        <label className="mb-3 block text-left">
          <span className="mb-1 block text-xs font-medium text-slate-600">
            {t('demoLogin.username')}
          </span>
          <input
            type="text"
            value={username}
        ...
        >
          {busy ? t('demoLogin.signingIn') : t('demoLogin.enter')}
        </button>
        ...
        <p className="mt-6 text-center text-xs text-slate-400">
          <Trans
            i18nKey="demoLogin.passwordHint"
            ns="auth"
            components={{ file: <code className="mx-1 rounded bg-slate-100 px-1" /> }}
          />
        </p>
      </form>
    </main>
