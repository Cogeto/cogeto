-- Migration 0025 — the task loop's last piece.
--
...
-- it — the derived memory's provenance points at this row, and the
...
--'s bar). The statement text is self-contained.
--
...
-- transient-extraction-input made durable).
...
CREATE TABLE task_conclusion (
  ...
  -- The deterministic, self-contained conclusion statement (
...
-- Idempotency belt: one conclusion per
