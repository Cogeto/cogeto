...
Hybrid, fused, filtered search (spec §3.4): semantic vectors (Qdrant) + keyword
...
- `scope` and `sensitive` are **hard gates** (WHERE-clause / Qdrant payload pre-filters
 inside the vector query, spec §4.2). App-side post-filtering of vector results is forbidden.
- Statuses are **score multipliers** on top of the gates (spec §3.4): active/user-approved ×1.0,
 uncertain ×0.6, contradicted ×0.4 with a visible warning, outdated ×0.2, replaced ×0.
- Temporal queries lift the outdated/replaced exclusion (spec §3.4, spec §6).
...
family `answer/` (spec §12.3), grounded strictly in retrieved facts.
