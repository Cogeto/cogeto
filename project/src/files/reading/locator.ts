 ...
 *
 * Nothing downstream of the reader consumes a locator yet: the pipeline
 * contract is unchanged and still carries `content: string`. What exists today
 * is the production side (every reader emits segments) and the resolution side
 * ({@link locateSpan}), which is what the golden cases assert and what the
 * V2.2 surfaces will call.
 */
