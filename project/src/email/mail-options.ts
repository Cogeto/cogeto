...
export interface MailOptions {
  ...
  /**
   * The bootstrap admin's email: the operator account is
   ...
   * rule. When true (the default, production), a message is only
   ...
   *. Bounds the ingestion/model spend an internet sender can drive on
   * the public inbound port. 0 disables the cap.
   */
  intakeMaxPerSenderPerWindow: number;
  /** The intake rate window, in seconds (). */
