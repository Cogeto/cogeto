 ...
 * PKCE flow. Written by the zitadel-init bootstrap job, served by the app.
 */
export interface WebConfig {
  issuer: string;
  clientId: string;
  /**
   * Ana sandbox. Present and true only on a demo instance
   * (`COGETO_DEMO_MODE=1`). The SPA shows the sandbox banner + first-visit
   * overlay.
   */
  demoMode?: boolean;
  /**
   * Password-gated sandbox login (revising 0022 ruling 1). True
