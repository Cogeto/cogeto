...
The authored fictional world for the `--profile demo` sandbox.
...
```jsonc
{
 "persona": { ... }, // documentation only: who is who
 "notes": [
 {
 ...
 "conversation": "..." // chat items only: the titled conversation this
...
- **`chat`** → first `POST /api/chat/conversations` + `PUT …/:id/title` once per
 distinct `conversation` title, then `POST /api/chat`
 (`{ content, conversationId }`, SSE), then
 `POST /api/chat/messages/:id/remember` (the explicit "remember this" flow), then poll `GET /api/chat/messages/:id/capture-status`.
...
- All fictional. No real person's data.
