...
- **Status**: the memory lifecycle state. **Six lifecycle statuses plus an
 orthogonal `sensitive` boolean flag**. Schema and code
...
- **Sensitive**: an orthogonal flag, not a status: `sensitive BOOLEAN NOT NULL
 ...
 owner, and only on explicit per-query opt-in.
...
- **Entrypoint**: a composition root producing a deployable process: **app**
 ...
 (all slow-path jobs). Same image, different entrypoints.
...
- **Conversation**: a chat workspace container: per-user
...
- **Skill**: a named, versioned, code-defined multi-step workflow with a
 declared plan of typed steps; registry-versioned like prompts
...
- **Open loops**: commitments and follow-ups that still stand. **Memory-backed
 since 2.0**: an open loop IS a memory whose `kind` is
