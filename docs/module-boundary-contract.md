...
| Owner | Tables |
|---|---|
| `memory` | `memory`, `embedding_index_state` (V2.4 item 7.1 second half: the vector index's durable state, one row, holding the active collection and dimension plus the managed rebuild in flight; the index is memory's, so its state is too), `memory_relation`, `memory_relation_event` (V2.3 item 6.1: the finding's append-only lifecycle history per [`docs/features/findings.md`](features/findings.md); the relation is the finding, so its event log lives with it and cascades with it), `file_metadata`, `deletion_receipt`, `integrity_alert` |
...
| `spaces` | `space`, `user_space_state`, `machine_space_binding` (session 4, section 6c: a machine caller's per-credential space binding, administrator-managed, CASCADE with its space) (V3 spaces session 1, decision record [`docs/features/spaces.md`](features/spaces.md): the sealed partition record and the user's last used space. The partition DIMENSION lives as a `space_id` column on every content-bearing root and inside both gate constructions; this module owns only the partition records themselves, and the gates read the id as a value off the Principal, never by joining these tables) |
...
| `confluence` | `confluence_page` (V2.5 item 8.2, the first real connector, decision record [`docs/features/confluence.md`](features/confluence.md): provenance per materialized source, page or attachment, so a fact traces to a specific version of a specific page and links back to it. Content-bearing, titles are the document's own words, so it joins the deletion cascade via `ConfluencePageCascade` and is erased with its source; the platform's ledger deliberately carries none of it) |
| `email` | `email_message`, `email_attachment`, `email_allowlist`, `email_refusal`, `email_alias` (V3 spaces session 4, [`docs/features/spaces.md`](features/spaces.md) section 6c: per-owner alias routing rules, mail to the plus-tagged inbound address lands in the named space; `email_allowlist` gained a `space_id` target the same session) |
...
| `connector_maintenance` | `connectors` | `CONNECTOR_MAINTENANCE_JOB_TYPE` | recurring (credential refresh ahead of expiry, webhook subscription renewal with the degrade-to-polling fallback, delivery-ledger prune, the periodic sync enqueue that IS the polling fallback, and the presence-sweep enqueue on each descriptor's cadence) |
| `connector.presence_sweep` | `connectors` | `CONNECTOR_PRESENCE_JOB_TYPE` | per-source (plain, single-flight per connector, V2.5 item 8.2 issue C5: page through the natural keys the upstream still lists, identifiers only, and reconcile the ledger, because polling by modified date cannot observe an absence; nothing is ever deleted) |
| `confluence.estimate` | `confluence` | `CONFLUENCE_ESTIMATE_JOB_TYPE` | per-source (the honest backfill estimate, V2.5 item 8.2 issue B2: one CQL count per scope for the connector's current settings, written to the sub-scope stats; counting needs the credential, which opens only in the worker) |
...
| `identity` | `PRINCIPAL`, `IDENTITY_OPTIONS`, `WEB_CONFIG_OPTIONS`, `MACHINE_SPACE_BINDINGS` (port, V3 spaces session 4: the guard's lookup of a machine caller's space binding; `spaces` implements it, the roots bind it, and its ABSENCE refuses every machine principal, fail closed) |
...
| `ingestion` | `SOURCE_READERS` (port), `PROJECT_POLICY` (port, V2.5 item 8.3: the per-project extraction policy. Ingestion declares it because the pipeline is the enforcement point; `projects` implements it; the worker root binds it, the `ingestionGuard` shape. Deliberately NOT a new gate dimension: it returns at most three numbers that fold into the same tightest-wins arithmetic every other bound uses, plus a disable that refuses through the existing metadata-only ledger with its own named reason), `RECONCILE_MODEL_CONFIG` (V2.3 item 6.1: the generation binding the checked-pair ledger records, handed in by the root) |
...
| `reports` | `REPORT_OPTIONS` (composition-root options: signing key dir, retention, the vendored font/brand/trust-score paths, and the model configuration in force), `REPORT_SPACE_NAMES` (port, V3 spaces session 4: the scope block's space-name lookup, the passport's resolver pattern; `spaces` implements it, the worker root binds it) |
