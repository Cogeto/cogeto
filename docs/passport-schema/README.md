...
| --- | --- | --- | --- |
| **2.1** | current | `manifest.json`, `memories.json`, `receipts.json` | The passport became per space: the manifest gained a required `space` field naming the one space the archive exports, and `receipts.json` carries the exporting owner's receipts in that space, each individually verifiable from the archive and the public key alone (the signature covers the payload including its `prev_hash` link). The space's chain is per-space from the shared genesis constant; a full genesis-to-tip walk from the archive alone additionally requires that every receipt in the space belongs to the exporting owner, since other owners' receipts are never exported. Hashing, signing and the receipt canonicalization are byte-identical to 2.0. |
