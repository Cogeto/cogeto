...
A claim becomes `active` only if the verdict is supported and the claim was not hedged in its source. Everything else becomes `uncertain`, carrying a distinguishable reason: `hedged_in_source`, `partially_supported`, `unsupported`, or `unjudgeable`. The mapping is total and has no default arm, and it is decided entirely by the machine: nothing blocks, nothing queues, and no admission waits for a person. The one case that is not admitted at all is `structurally_invalid`, a blank claim or a blank span, where storing would be actively wrong rather than merely uncertain. Every demotion and every non-admission writes a **suppressed fact log** entry with fact, source, exact span, reason, verification detail and timestamp, gated exactly as memories are, queryable by source, reason and date range, shown on the source detail, and summarized in the findings report. Because it is content bearing it is enumerated by the deletion saga and counted in the receipt.
