...
| Subcommand | Effect |
| --- | --- |
| `install` | Fresh Ubuntu 22.04/24.04 → running instance: OS/resource preflight, Docker Engine + compose plugin (official apt repo), **cosign** (pinned release binary; signature verification of every image this instance runs, which is three plus the redaction sidecar when that capability is on), **the script itself to `/usr/local/bin/cogeto`**, `cogeto` system user, `/srv/cogeto` layout, the release's deployment-assets artifact downloaded and verified (outer checksum, then per-file manifest), per-instance secrets into `.env` (600), the derived inbound address (`capture@in.<domain>`), signed-image pull, `docker compose up -d`, health wait, checklist. |
...
| `upgrade [X.Y.Z\|latest]` | Self-heals the `/usr/local/bin/cogeto` install first (issue #60: a freshly downloaded script run via `upgrade` must leave `sudo cogeto` working). Decides what the instance is on from the **running app image**, not from `COGETO_VERSION` in `.env`, and records the new version only **after** `compose up` succeeds: an upgrade that dies at the pull or the signature check leaves the file telling the truth and is RESUMABLE by re-running the same command (a version written ahead of the work it describes made a failed upgrade look finished and refuse the retry, observed at v1.7.3). A pull, signature or start failure names what failed, what to do, and that nothing changed. Published-tag check (Docker Hub), typed confirmation, fetch and verify the target version's deployment artifact, pull, `compose up -d` (the dependency graph re-runs preflight → migrate before app/worker restart), health check, embedding-model drift check → `reindex` (typed confirmation, it re-embeds via the model API), rollback instructions, then **image cleanup**: Cogeto image tags other than the new version and the one it replaced are removed with a non-forced `docker rmi` (a container still referencing one keeps it, and that refusal is reported). Infra images are digest-pinned and never touched, and the previous release stays on disk precisely so the rollback below needs no network. Rollback = `upgrade <older>` with a ROLLBACK confirmation; schema stays forward, data rollback is the provider-backup restore. |
...
| `backup-info` | Exactly what to enable at your cloud provider (the script performs no backups). |
...
`cogeto/cogeto-redaction` when that capability is on, and downloads the
matching release's **deployment-assets artifact**
(`cogeto-deploy-assets-X.Y.Z.tar.gz`), which carries the compose file, the
Caddyfile, the Zitadel bootstrap script, the Postgres role provisioning and
the SearXNG settings, together with the per-file checksum manifest and a
`VERSION` entry. `stage_deploy_artifact` verifies the tarball against the
sha256 the release publishes beside it, unpacks it, checks the version marker
and verifies every file against the manifest inside before `install_asset`
puts any of them in place: [`../release-process.md`](../release-process.md).
...
developer pass on a fresh Ubuntu 24.04 server is:
...
3. `sudo ./cogeto install --domain <instance domain> --acme-email <you>`:
 ...
 capture on), enable the provider's instance backup.
