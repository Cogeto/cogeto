...
version table state staleness as fact and advisory exposure as unverified; GitHub
...
as *enforced* — an immutable audit trail and an isolated identity boundary — are
enforced only by the app choosing not to. The per-module "no cross-module table
...
exposure — the defect is the *credential*, and the vhost is the amplifier. The app
...
Which endpoint: exactly one — `GET /api/health`. `GET /api/health/live` is fine and
