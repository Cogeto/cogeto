# Cogeto v1 Addendum: architecture decisions and verifiable memory
...
- **worker**: all slow-path jobs (extraction, dedup, contradiction checks, consolidation/dreaming, deletion sagas). *(2.0: reminders removed:.)*

Both are built from one codebase organized as **DDD bounded contexts** (modules): `memory` (core domain), `ingestion`, `retrieval`, `agents`, `connectors`, `identity` (Zitadel seam), `model-gateway` (Mistral seam). *(2.0: the `tasks` context was removed,; open loops are memory-backed.)* Module rules, enforced by import-linting / architecture tests in CI:
...
- **Golden set:** 50 to 100 hand-labeled notes/emails/events with expected memories (per language served). Grows with every real-world failure case.
